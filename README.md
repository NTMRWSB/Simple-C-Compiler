...
1. 运用lex和yacc工具进行词法分析和语法分析，产生中间代码文件Innercode。
2. 使用C++从中间代码生成汇编语言，运用C++中STL库的map、set等容器，从中间代码文件高效地转化为汇编代码。
3. 可输出移入—归约冲突和归约—归约冲突，也可以设置优先级以避免冲突。
4. 重构了语法树的代码，改为抽象语法树，结构更加简明。
5. 将原本的链表符号表改为链式哈希表，确定好变量的定义域和重定义问题。
